...
full_tasks = ['fetch_recorded_demo_1730997119.h5']
ground_truths = [[0, 1125, 2591, 3986, 5666]]
        ...
        for j in range(len(norms)):
            for k in range(len(sim_metrics)):
                for l in range(len(full_task_data)):
                    ...
                    print(np.shape(s_data[0]), np.shape(s_data[1]), np.shape(s_data[2]), np.shape(s_data[3]), np.shape(s_data[4]))
                    ...
                    print(gt[2], gt[3], classes[gt[2]], classes[gt[3]], st3_tru)
                    ...
                    string = 'smooth' if i == 1 else 'unsmooth'
                    string = string + '_' + sim_metrics[k] + '_'
                    
                    idx = idx + 1
                    np.savetxt('seg_acc5.txt', accs)
                    np.savetxt('seg_inds5.txt', inds)
                    np.savetxt(string + 'Z_mat.txt', classes)
    print(accs)
    print(inds)